...
[PostgreSQL Docs](https://www.postgresql.org/docs/) - Make sure to use the documentation that matches the version of PostgreSQL you have installed
...
For this lab assignment, you will be persisting your data in a SQL database. Upon each request, check the database for the records that match the search query. If the records exist, you will check how old they are and determine if those records should be used. If the records do not exist, you will request them from the API.
...
- Follow the same code review process as lab 7, carrying over your `server.js` and `README.md` files.
...
- You will also need to provision a SQL database on Heroku. To do so, nagivate to the Resources tab and in the Add-Ons, search for "Postgres" and provision the free version. Verify that this was successful by clicking on the Settings tab and revealing your config vars. You will see that Heroku has automatically created a DATABASE_URL variable.
...
Table creation:
- Within your SQL shell, create a database to hold your API data. For example, if you want to make a database named `city_explorer`, use the command `CREATE DATABASE city_expolorer;`.
...
- Push your database to your Heroku instance with the following format: `heroku pg:push <database-name> DATABASE_URL --app <heroku-instance-name>`.
- Redeploy your application.
- When you switch driver-navigator roles, you may want to pull your remote (Heroku) database to your computer. You can do this with the pg:pull command in this format: `heroku pg:pull DATABASE_URL <database-name> --app <heroku-instance-name>`. If you typically need to include a username and password, see the [Heroku documentation](https://devcenter.heroku.com/articles/heroku-postgresql) for pg:pull for more details.
...
- Update each model to include a new property to keep track of when the record was added to the database. If necessary, drop your tables and re-create them to include this new property. Update the save function for each model.
